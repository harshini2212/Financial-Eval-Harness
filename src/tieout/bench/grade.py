...
def llm_judge(q: dict, ans, judge_model: str, cache) -> bool:
    """The naive LLM-as-judge baseline Rogo's BFB critiques: it grades the FINAL
    ANSWER for plausibility, with no answer key and no derivation to inspect — so a
    confident, plausible-but-wrong answer gets rubber-stamped. The constraint layer
    catches exactly these (the money metric)."""
